...
import { closeSurugaya } from '../core/adapters/index.js';
import { crawl, liveSearch } from '../core/crawler.js';
import { addWatch, getMeta, listWatches, queryItems, removeWatch, setMeta, stats } from '../core/db.js';
import { toast } from '../core/notify.js';
import { SOURCE_NAMES, type Source } from '../core/types.js';

const program = new Command();
program.name('bandori').description('BanG Dream! 中古情報聚合 CLI');
...
function printTable(items: { source: string; price: number | null; status: string; title: string; url: string }[]) {
  for (const it of items) {
    const src = SOURCE_NAMES[it.source as Source] ?? it.source;
    const st = it.status === 'soldout' ? '✗售完' : it.status === 'instock' ? '✓有貨' : '?';
    console.log(`${src.padEnd(10)} ${fmtPrice(it.price).padStart(9)} ${st}  ${it.title.slice(0, 60)}`);
    console.log(`  ${it.url}`);
  }
}
...
program
  ...
  .option('-s, --site <sites>', '限定站點 (surugaya,lashinbang,hardoff,bookoff,kbooks,mercari)')
  .option('-p, --pages <n>', '每站頁數', '1')
  .option('--local', '只查本地資料庫，不打網站')
  .option('--json', '輸出 JSON')
  .description('即時搜尋各站（結果同時寫入資料庫）')
  .action(async (kwParts: string[], o) => {
    const keyword = kwParts.join(' ');
    const sources = o.site ? (o.site.split(',') as Source[]) : undefined;
    if (o.local) {
      const { items, total } = await queryItems({ q: keyword, source: sources, limit: 100 });
      if (o.json) console.log(JSON.stringify(items, null, 2));
      else {
        console.log(`資料庫命中 ${total} 件（顯示前 ${items.length}）\n`);
        printTable(items);
      }
      return;
    }
    console.error(`即時搜尋「${keyword}」…（各站禮貌節流，約需 5~20 秒）`);
    const { items, errors } = await liveSearch(keyword, { sources, pages: Number(o.pages) });
...
program
  ...
  .action(async (o) => {
    const report = await crawl({
      full: o.full,
      quiet: o.quiet,
      sources: o.site ? (o.site.split(',') as Source[]) : undefined,
      keywords: o.keywords ? o.keywords.split(',') : undefined,
    });
    console.log('\n=== 掃站結果 ===');
...
program
  .command('auth')
  .description('測試駿河屋驗證流程（注意：驗證只在同一個執行的 session 內有效；掃站/搜尋時視窗會自動開啟）')
  .action(async () => {
    console.log('開啟瀏覽器中… 若出現挑戰頁，請點「私はロボットではありません」勾選框。');
    const { authSurugaya } = await import('../core/adapters/surugaya.js');
    const ok = await authSurugaya();
    console.log(ok ? '✓ 驗證通過！' : '✗ 未在時限內通過驗證，請再試一次。');
    await closeSurugaya();
    process.exit(ok ? 0 : 1);
  });
...
watch
  ...
  .action(async (kw: string[]) => {
    await addWatch(kw.join(' '));
    console.log(`✓ 已關注「${kw.join(' ')}」`);
  });
watch.command('ls').action(async () => {
  for (const w of await listWatches()) console.log(`#${w.id}  ${w.keyword}`);
});
watch
  .command('rm')
  .argument('<id>')
  .action(async (id: string) => {
    await removeWatch(Number(id));
    console.log('✓ 已移除');
  });
...
program
  ...
  .description('列出願望清單')
  .action(async () => {
    const { items } = await queryItems({ wished: true, limit: 200 });
    printTable(items);
  });
...
program
  ...
  .action(async (key: string, value?: string) => {
    if (value == null) console.log((await getMeta(key)) ?? '(未設定)');
    else {
...
program
  .command('notify-test')
  .description('測試通知（toast + discord）')
  .action(async () => {
    toast('Bandori Hunter', '通知測試 🎸');
    const { discord } = await import('../core/notify.js');
    await discord('通知測試 🎸');
    console.log('已送出。');
  });
...
program
  ...
  .action(async (o) => {
    ...
    startServer({ port: Number(o.port), schedule: o.schedule });
  });
